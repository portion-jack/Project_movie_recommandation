...
def recommand_user_id_svd(df_main,user_id):
    ...
    temp=pd.DataFrame(preds).drop(columns=['r_ui','details','uid']).groupby('iid').mean('est').nlargest(100,'est').reset_index()
...
def recommand_user_id_movie(df_main,user_id):
    ...
    for seen_movie in seen_movies:
        ...
        cosine_movie_sim.append(movie_user_simDF.loc[movie_user_simDF.index != movie_name,:][movie_name].nlargest(10))

    result2=pd.concat(cosine_movie_sim).reset_index().groupby('title').sum().nlargest(100,0)
...
def recommand_user_id_genres(df_main,user_id):
    ...
    result=merged_recommand[merged_recommand['genres'].apply(lambda x: True if len(set(genres)-set(x))==0 else False)].nlargest(100,'est').reset_index(drop=True)
...
def recommand_user_id_ensemble(df_main,user_id,n):
    ...
    recommands=result.groupby('title').sum().nlargest(n,'point')
    return recommands